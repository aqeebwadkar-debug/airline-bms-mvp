...
export function FlightsScreen({ onOpenFlight }: Props) {
  ...
  const filtered = useMemo(() => {
    return MOCK_FLIGHTS.filter((f) => {
      ...
      const matchesDate = f.stdLocal.startsWith(date);
      return matchesQ && matchesAir && matchesSt && matchesDate;
    });
  }, [query, airline, status, date]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs text-slate-500">
            Network flight baggage operations
          </p>
        </div>
        ...
        <PanelBody className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          ...
          <label className="space-y-1">
            ...
            <select
              value={status}
              onChange={(e) => {
                setStatus(e.target.value as FlightStatus | "All");
                resetPage();
              }}
              className="h-9 w-full rounded-md border border-slate-200 bg-white px-2 text-xs outline-none ring-blue-500/30 focus:ring-2"
            >
      ...
      <Panel>
        <PanelHead
          title="Flight control table"
          subtitle="Compact operational density — click flight for detail"
        />
        {filtered.length === 0 ? (
          <PanelBody>
            <EmptyState
              title="No flights for filters"
              hint="Adjust date or clear status filters."
            />
          </PanelBody>
        ) : (
          <>
